...
const schema = z.object({
  ...
  witnessCpf: z.string().regex(/^\d{3}\.\d{3}\.\d{3}-\d{2}$/, 'CPF da testemunha inválido'),
...
export const InitialDataForm: React.FC<InitialDataFormProps> = ({ onSubmit, loading }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-gray-700 mb-1">Nome Acadêmico</label>
            <input
      ...
      {/* Witness Info */}
      <div className="bg-white p-6 rounded-lg shadow-sm">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Dados da Testemunha</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Nome da Testemunha *</label>
            <input
              {...register('witnessName')}
              className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
            {errors.witnessName && (
              <p className="text-red-500 text-sm mt-1">{errors.witnessName.message}</p>
            )}
          </div>
          
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Email da Testemunha *</label>
            <input
              type="email"
            ...
            />
            {errors.witnessEmail && (
              <p className="text-red-500 text-sm mt-1">{errors.witnessEmail.message}</p>
            )}
          </div>
          
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">CPF da Testemunha *</label>
            <input
              {...register('witnessCpf')}
              placeholder="000.000.000-00"
              className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
            {errors.witnessCpf && (
              <p className="text-red-500 text-sm mt-1">{errors.witnessCpf.message}</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Tipo de Parceria *</label>
            <select
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Captado Por *</label>
            <input
              {...register('capturedBy')}
              className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
            {errors.capturedBy && (
              <p className="text-red-500 text-sm mt-1">{errors.capturedBy.message}</p>
            )}
          </div>
        </div>
      </div>
