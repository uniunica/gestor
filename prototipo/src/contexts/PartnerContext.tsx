...
export const PartnerProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const updateFinalRegistration = async (
    partnerId: string,
    data: FinalRegistration
  ): Promise<void> => {
    setLoading(true);
        ...
        prev.map((partner) =>
          partner.id === partnerId
            ? {
                ...
                trainingData: data,
                currentStage:
                  data.status === "completed" ? "completed" : "training",
                updatedAt: new Date(),
              }
            : partner
        )
  ...
  const filterPartners = (filters: PartnerFilters): Partner[] => {
    return partners.filter((partner) => {
      ...
      if (filters.city && partner.initialData.city !== filters.city)
        return false;
      if (filters.state && partner.initialData.state !== filters.state)
        return false;
      if (
        filters.partnershipType &&
        partner.initialData.partnershipType !== filters.partnershipType
      )
        return false;
      ...
      if (filters.dateRange) {
        ...
        if (
          partnerDate < filters.dateRange.start ||
          partnerDate > filters.dateRange.end
        )
          return false;
      }
