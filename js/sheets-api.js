...
class SheetsAPI {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (
          ...
          this.apiEnabled = false;
          console.warn(
            "Google Sheets API não está habilitada. Usando dados locais."
          );
        }
  ...
  shouldTryAPI() {
    ...
    if (
      !this.apiEnabled &&
      Date.now() - this.lastApiCheck < this.apiCheckInterval
    ) {
      return false;
    }
    return true;
  ...
  async readSheet(sheetName, range = "") {
    try {
      ...
      console.log(
        `Erro ao acessar Google Sheets para ${sheetName}, usando dados locais`
      );
      return this.getLocalData(sheetName);
    }
  }
  ...
  async testConnection() {
    try {
      ...
      if (
        ...
        toast.warning(
          "⚠️ Google Sheets API não habilitada. Habilite a API e recarregue a página. Usando dados locais por enquanto."
        );
      } else {
        toast.error(
          "❌ Erro ao conectar com Google Sheets. Usando dados locais."
        );
      }
