...
class SheetsAPI {
    constructor() {
        this.apiKey = 'SUA_API_KEY_AQUI'; // Substitua pela sua API Key
        this.spreadsheetId = 'SEU_SPREADSHEET_ID_AQUI'; // Substitua pelo ID da sua planilha
    ...
    async searchParceiros(query, filters = {}) {
        try {
            ...
            if (filters.tipo) {
                filteredData = filteredData.filter(item => 
                    item.tipoParceria === filters.tipo
                );
            }
    ...
    async getStats() {
        try {
            ...
            const stats = {
                ...
                emTreinamento: dadosTreinamento.filter(item => 
                    item.status === 'Em Treinamento'
                ).length,
                parceirosAtivos: dadosTreinamento.filter(item => 
                    item.status === 'Ativo'
                ).length,
                porTipo: {},
                porUF: {}
